...
const newEmailAddress = username.concat("@", newDomain);//use a string method and the oldDomain and newDomain variables to create the new email address as described in the starting comments
console.log(`The new email address is ${newEmailAddress}`);

const first5 = username.slice(0,4);
const origLen = emailAddress.length

const tempPassword = first5.concat(origLen);//use string methods and either concatenation or interpolation to generate the temporary password as described in the starting comments, you might find it useful to create another variable above this line to do this
